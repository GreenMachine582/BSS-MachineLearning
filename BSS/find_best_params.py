...
def getGradientBoostingRegressor(random_seed):
    ...
    #                 'max_depth': [1, 3, 10, 50]}
    return GradientBoostingRegressor(random_state=random_seed), param_search


def getMLPRegressor(random_seed):
    param_search = {"hidden_layer_sizes": [(1,), (50,)],
                    "activation": ["identity", "logistic", "tanh", "relu"],
                    "solver": ["lbfgs", "sgd", "adam"],
                    "alpha": [0.00005, 0.0005]}
    return MLPRegressor(max_iter=1200, random_state=random_seed), param_search
...
def main(dir_: str = local_dir):
    ...
    while True:
        ...
        if choice is not None:
            if choice == 0:
                ...
                model, param_search = getGradientBoostingRegressor(config.random_seed)
                break
            elif choice == 2:
                model, param_search = getMLPRegressor(config.random_seed)
                break
            elif choice == 3:
    ...
    model = ml.Model(config.model, estimator=model)
