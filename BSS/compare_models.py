...
def compareModels(models: dict, X_train: DataFrame, y_train: Series, dataset_name: str = '',
    ...
    :param results_dir: Save location for figures and results, should be a str
...
def compareEstimators(dataset: Dataset, config: Config) -> None:
    ...
    :return: None
    """
    models = {'GBR': ensemble.GradientBoostingRegressor(random_state=config.random_state),
    ...
    results = compareModels(models, X_train, y_train, dataset_name=dataset.name, results_dir=results_dir)
    ...
    ml.estimator.plotPrediction(y_train.resample('D').sum(), y_test.resample('D').sum(), y_preds, ylabel='Demand',
